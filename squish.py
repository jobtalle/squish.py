...
class Document:
    ...
    @staticmethod
    def __compress(directory, contents, tag, css_variables):
        ...
        if tag == 'script':
            return compress_js(directory, contents, css_variables)
        elif tag == 'style':
    ...
    def __collect(self, tags):
        ...
        def get_match(match):
            ...
            if last_match[tag] is not None:
                if last_match[tag].span()[1] == match.span()[0]:
                    ...
                    return self.__make_tag(
                        include_tag[tag],
                        self.__compress(directory, script_contents, include_tag[tag], css_variables))
            else:
                combined[tag] = source_contents
            ...
            if match.span()[0] == last_index[tag]:
                return self.__make_tag(
                    include_tag[tag],
                    self.__compress(directory, combined[tag], include_tag[tag], css_variables))
